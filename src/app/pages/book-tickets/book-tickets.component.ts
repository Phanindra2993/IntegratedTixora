...
export class BookTicketsComponent {
  ...
  confirmBooking() {
    ...
    // this.movieService.bookTickets(bookingPayload).subscribe({
    //   next: (res) => {
    //     // Create the ticket data object with all required properties
    //     const ticketData = {
    //       bookingId:
    //         res.bookingId || 'BK' + Math.floor(100000 + Math.random() * 900000),
    //       userId: loggedInUser.userId,
    //       movieTitle: this.movie.title,
    //       movieImage: this.movie.imageUrl,
    //       showDate: this.selectedDate,
    ...
    //       format: this.movie.format,
    //       totalAmount: res.totalAmount || this.seatCount * 200,
    //       // Add any other properties your ticket component expects
    //     };
    ...
    this.movieService.bookTickets(bookingPayload).subscribe({
      ...
      error: (err) => {
        ...
        // Specific handling for duplicate booking error
